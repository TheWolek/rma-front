...
export default {
  ...
  methods: {
    onAdd() {
      if (this.codeToAdd != "") {
        if (!this.reg.test(this.codeToAdd))
          return this.onFail("niepoprawny format");
        if (!this.allowedProducts.includes(this.codeToAdd))
          return this.onFail("brak produktu");
        if (this.codeAlreadyEntered(this.codeToAdd))
          return this.onFail("kod już wprowadzony");
